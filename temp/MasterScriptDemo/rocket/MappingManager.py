...
from UrsiFunctions import BirthdateByUrsi
from UrsiFunctions import GenderByUrsi
from UrsiFunctions import GuidByUrsi
...
class SinkCol:
	...
	def calculate_data(self, data_list, source_missing_val):
		...
		if self.function == 'findGuid':
			guidfinder = GuidByUrsi(data_list =  data_list)
			self.data = guidfinder.find()

		if self.function == 'findBirthdate':
			birthdatefinder = BirthdateByUrsi(data_list =  data_list)
			self.data =birthdatefinder.find()

		if self.function == 'findGender':
			genderfinder = GenderByUrsi(data_list = data_list)
			self.data = genderfinder.find()
