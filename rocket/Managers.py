...
class ssManager(Manager):
    ...
    def load_template(self, templ_csv_file) -> None:
...
class sinkManager(ssManager):
    ...
    def read_output_file(self) -> Tuple[str, TextIO]:
        ...
        # Let's see whether I can link this with automatical update
    ...
    def _auto_generate_file_path_(self):
        ...
        :return:
        """
        return "rocket_output_{time}.csv".format(time=datetime.now().strftime("%b_%d_%y_%H"))

    def get_file_outpath(self, allownew=False, operated_manually=True):
        """
        ...
        :param allownew:
        :param operated_manually: Set this flag to false allow rocket always use auto-generated stuff
        :return:
        """
        ...
        if operated_manually:
            ...
            p = Path(self.defaultdatadir, self._auto_generate_file_path_())
            self.outpath = str(p)
